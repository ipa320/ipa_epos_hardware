            ...
            BOOST_FOREACH (const std::string& interface_name, interface_names)
            {
              ...
              if (GetPortNameList(device_name, protocol_stack_name, interface_name, &port_names, &error_code))
              {
                BOOST_FOREACH (const std::string& port_name, port_names)
                {
                  ...
                  if (GetBaudrateList(
                          device_name, protocol_stack_name, interface_name, port_name, &baudrates, &error_code))
                  {
                    ...
                    if (epos_factory.EnumerateNodes(
                            device_name, protocol_stack_name, interface_name, port_name, &devices, &error_code))
                    {
                      ...
                      BOOST_FOREACH (const EnumeratedNode& node, devices)
                      {
                        ...
                        std::cout << "\t\t\t\t\t\tApplication Number: 0x" << std::hex << node.application_number
                                  << std::endl;
                        std::cout << "\t\t\t\t\t\tApplication Version: 0x" << std::hex << node.application_version
                                  << std::endl;
                      }
                    }
                    else
