                             ...
                             unsigned int* error_code)
{
  ...
  result = VCS_GetProtocolStackNameSelection(
      (char*)device_name.c_str(), true, buffer, MAX_STRING_SIZE, &end_of_selection, error_code);
  if (!result)
    return result;
  ...
  {
    result = VCS_GetProtocolStackNameSelection(
        (char*)device_name.c_str(), false, buffer, MAX_STRING_SIZE, &end_of_selection, error_code);
    if (!result)
      return result;
    protocol_stack_names->push_back(buffer);
  }
...
NodeHandlePtr EposFactory::CreateNodeHandle(const EnumeratedNode& node, unsigned int* error_code)
{
  DeviceHandlePtr device_handle =
      CreateDeviceHandle(node.device_name, node.protocol_stack_name, node.interface_name, node.port_name, error_code);
  if (!device_handle)
    return NodeHandlePtr();
