...
Epos::Epos(const std::string& name,
  ...
  , diagnostic_updater_(nh, config_nh)
  , epos_factory_(epos_factory)
  , has_init_(false)
  , position_(0)
  , velocity_(0)
  , effort_(0)
  ...
  , sensor_resolution_(1)
{
  ...
  if (!config_nh_.getParam("hw_version", hw_version_))
  {
    ROS_ERROR("You must specify a hardware_version (EPOS2 or EPOS4)");
    valid_ = false;
  }

  std::string serial_number_str;
  if (!config_nh_.getParam("serial_number", serial_number_str) && !config_nh_.getParam("port_name", port_name_))
  {
    ROS_ERROR("You must specify a serial number or a port name");
    valid_ = false;
  }
  else if (config_nh_.getParam("serial_number", serial_number_str))
  {
    use_serial_number_ = true;
    ROS_ASSERT(SerialNumberFromHex(serial_number_str, &serial_number_));
  }
  else if (config_nh_.getParam("port_name", port_name_))
  {
...
public:
  ...
  template <class T>
  ParameterSetLoader& param(const std::string& name, T& value)
  {
...
bool Epos::init()
{
  if (!valid_)
  {
    ...
    {
      ROS_ERROR_STREAM("Not Initializing motor at port: " << port_name_ << ", initial construction failed");
    }
  ...
  if (use_serial_number_)
  {
    ...
    node_handle_ = epos_factory_->CreateNodeHandle(hw_version_, "MAXON SERIAL V2", "USB", serial_number_, &error_code);
  }
  else
  {
    ROS_INFO_STREAM("Initializing: " << port_name_);
    node_handle_ = epos_factory_->CreateNodeHandle(hw_version_, "MAXON SERIAL V2", "USB", port_name_, &error_code);
  }
  if (!node_handle_)
  {
...
void Epos::read()
{
  ...
  VCS_GetObject(
      node_handle_->device_handle->ptr, node_handle_->node_id, 0x6041, 0x00, &statusword_, 2, &bytes_read, &error_code);
...
void Epos::buildMotorStatus(diagnostic_updater::DiagnosticStatusWrapper& stat)
{
  ...
  if (has_init_)
  {
    bool enabled = STATUSWORD(READY_TO_SWITCH_ON, statusword_) && STATUSWORD(SWITCHED_ON, statusword_) &&
                   STATUSWORD(ENABLE, statusword_);
    if (enabled)
    {
    ...
    if (VCS_GetNbOfDeviceError(node_handle_->device_handle->ptr, node_handle_->node_id, &num_errors, &error_code))
    {
      ...
      {
        unsigned int error_number;
        if (VCS_GetDeviceErrorCode(
                node_handle_->device_handle->ptr, node_handle_->node_id, i, &error_number, &error_code))
        {
